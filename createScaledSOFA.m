...
allResults = combineAllResults(1000,directory);

% Load in Generic KU100 HRTF
hrtf = SOFAload('D1_44K_16bit_256tap_FIR_SOFA.sofa');
hrtfExpanded = SOFAexpand(hrtf);

for file = 1:size(allResults.leftAverage)
    
    ILDLeftEstimated = allResults.leftAverage(file);
    ILDRightEstimated = allResults.rightAverage(file);
    ...
    fileName = sprintf('%d_D1_ILD_Scaled.sofa',file);
    SOFAsave(fileName,hrtfScaled,0);
end